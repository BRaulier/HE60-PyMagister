...
if [ "`uname`" == "Darwin" ]; then
  BUILD_DIR=../build_osx/hl_build
else 
  BUILD_DIR=../build_linux/hl_build
fi

mkdir -p $BUILD_DIR

# in a normal installation the backend directory is two directories up
if [ -d "../../backend" ]; then
...
cp spawnHL_stnd.sh $BUILD_DIR"/"$REL_TARG_DIR
...
if [ "`uname`" == "Darwin" ]; then
  sed -e "s|\\\\frontend\\\\|/MacOS/|g" main_HydroLight.f95 > main_HydroLight_var.f95 
else
  sed -e "s|\\\\frontend\\\\|/frontend/|g" main_HydroLight.f95 > main_HydroLight_var.f95 
fi
...
gfortran $OPTS -c ../../HydroLight_code/select_pf_bb_mod.f95
...
# HydroLight core code modules in the order required by dependencies
gfortran $OPTS -c ../../HydroLight_code/initialize_printout_mod.f95
gfortran $OPTS -c ../../HydroLight_code/print_grid_mod.f95
gfortran $OPTS -c ../../HydroLight_code/quad_average_sky_mod.f95
gfortran $OPTS -c ../../HydroLight_code/initialize_mod.f95
gfortran $OPTS -c ../../HydroLight_code/load_surface_mod.f95
gfortran $OPTS -c ../../HydroLight_code/quad_average_BRRF_mod.f95
gfortran $OPTS -c ../../HydroLight_code/rho_tau_mod.f95
gfortran $OPTS -c ../../HydroLight_code/infinite_bottom_mod.f95
gfortran $OPTS -c ../../HydroLight_code/bottom_bndcond_mod.f95

gfortran $OPTS -c ../../HydroLight_code/amplitudes_direct_mod.f95
gfortran $OPTS -c ../../HydroLight_code/amplitudes_zeta_mod.f95
gfortran $OPTS -c ../../HydroLight_code/amplitudes_w_mod.f95
gfortran $OPTS -c ../../HydroLight_code/amplitudes_waterleaving_mod.f95
gfortran $OPTS -c ../../HydroLight_code/initialize_amplitudes_mod.f95

gfortran $OPTS -c ../../HydroLight_code/dRTSdzeta_mod.f95
gfortran $OPTS -c ../../HydroLight_code/Riccati_mod.f95

gfortran $OPTS -c ../../HydroLight_code/print_amplitudes_mod.f95
gfortran $OPTS -c ../../HydroLight_code/radiance_amplitudes_mod.f95
gfortran $OPTS -c ../../HydroLight_code/synthesize_radiances_mod.f95
gfortran $OPTS -c ../../HydroLight_code/irradiances_mod.f95
gfortran $OPTS -c ../../HydroLight_code/print_radiances_mod.f95
gfortran $OPTS -c ../../HydroLight_code/write_Lfile_mod.f95
gfortran $OPTS -c ../../HydroLight_code/write_LEfile_mod.f95
gfortran $OPTS -c ../../HydroLight_code/write_Dfile_mod.f95
...
if [ "`uname`" == "Darwin" ]; then
  ...
  # gfortran $OPTS ../../HydroLight_code/main_HydroLight_var.f95 *.o $REL_TARG_DIR/heinfof.so -o $REL_TARG_DIR/HydroLight6
  gfortran $OPTS ../../HydroLight_code/main_HydroLight_var.f95 *.o -o $REL_TARG_DIR/HydroLight6
  ...
  if [ "$1" == "USE_BUNDLED_LIB" ]; then
    ...
    # no longer needed
    # install_name_tool -change heinfof.so @executable_path/heinfof.so HydroLight6

    mkdir -p lib

    # some of the following is duplicated in the EcoLight build script
    # but it means that either script can work as stand-alone

    # copy these even in a local build so if gfortran is uninstalled HE60 will still run
    ...
    install_name_tool -change /usr/local/gfortran/lib/libgcc_s.1.dylib @executable_path/lib/libgcc_s.1.dylib HydroLight6
    install_name_tool -change /usr/local/gfortran/lib/libgfortran.3.dylib @executable_path/lib/libgfortran.3.dylib HydroLight6 
    install_name_tool -change /usr/local/gfortran/lib/libquadmath.0.dylib @executable_path/lib/libquadmath.0.dylib HydroLight6 
...
# linux build
  ...
  # can check rpath with readelf -d and actual runtime linkage with ldd
  ...
  if [ "$1" == "USE_BUNDLED_LIB" ]; then
    ...
    # gfortran $OPTS -Wl,-rpath,'$ORIGIN:$ORIGIN/../lib' ../../HydroLight_code/main_HydroLight_var.f95 *.o heinfof.so -o $REL_TARG_DIR/HydroLight6
    gfortran $OPTS -Wl,-rpath,'$ORIGIN:$ORIGIN/../lib' ../../HydroLight_code/main_HydroLight_var.f95 *.o -o $REL_TARG_DIR/HydroLight6
  else
    # ln -sf $REL_TARG_DIR/heinfof.so
    # gfortran $OPTS -Wl,-rpath,'$ORIGIN' ../../HydroLight_code/main_HydroLight_var.f95 *.o heinfof.so -o $REL_TARG_DIR/HydroLight6
    gfortran $OPTS -Wl,-rpath,'$ORIGIN' ../../HydroLight_code/main_HydroLight_var.f95 *.o -o $REL_TARG_DIR/HydroLight6
  fi
