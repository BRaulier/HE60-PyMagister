...
def compute_transmittance_from_Kd(Kd_z, delta_z=0.005):
    transmittance = np.exp(-np.sum(delta_z*Kd_z))
...
class DataFinder:
    ...
    def get_broadband_transmittance(self):
        ...
        self.transmittance = compute_transmittance_from_Kd(Kd_z=self.broad_band_Kdz_df['K_d (broadband)'])
